    ...
    <div className="flex-1 flex items-center justify-center p-4 min-h-screen relative overflow-hidden bg-[#FAFAFA]">
      ...
      <div className="max-w-md w-full bg-white p-10 rounded-[24px] border border-zinc-200/80 shadow-[0_24px_60px_-12px_rgba(0,0,0,0.08)] relative z-10">
        <div className="text-center mb-10">
          ...
          <h2 className="text-[14px] text-zinc-500 font-medium">{t.auth.login_title}</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-[11px] font-bold text-zinc-500 uppercase tracking-widest mb-2">{t.auth.email}</label>
            <input 
              type="email" 
              required
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-[11px] font-bold text-zinc-500 uppercase tracking-widest">{t.auth.password}</label>
        ...
        <p className="mt-8 text-center text-[13px] text-zinc-500 font-medium">
          {t.auth.no_account} <Link to="/register" className="text-indigo-600 font-bold hover:text-indigo-700 transition-colors">Register</Link>
        </p>
      </div>
    </div>
  );
